...
#if defined(_WIN32)
static CRITICAL_SECTION s_PortingCriticalSection;
static bool s_PortingCriticalSectionInitialized = false;
...
extern __declspec(dllexport) LONGLONG __cdecl __PORTING__InterlockedExchangeAdd64(
  ...
  _In_     LONGLONG Value
	...
	LONGLONG result;
	if( !s_PortingCriticalSectionInitialized )
	{
		s_PortingCriticalSectionInitialized = true;
		InitializeCriticalSection(&s_PortingCriticalSection);
	}
	EnterCriticalSection(&s_PortingCriticalSection);
	result = *Addend;
	*Addend += Value;
	LeaveCriticalSection(&s_PortingCriticalSection);
	return result;
}
#endif